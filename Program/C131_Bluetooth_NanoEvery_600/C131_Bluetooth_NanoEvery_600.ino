...
class Runner
{
    public:
        ...
        void runAutomatic()
        {
            ...
            if (!mover.isStopped())
            {
                if (mover.getState() != Mover::Correction && millis() - _timer2 >= 50 * timeMultiplier)
                {
            ...
            if (_currentState == Beginning && millis() - _timer >= 10 * timeMultiplier) // 10 ms is a time for preparing camera
            {
                _stepNumber++;
            ...
            if (_currentState == Exposure && millis() - _timer >= Settings::getExposure() * timeMultiplier)
            {
            ...
            {
                if (millis() - _timer2 >= 1000 * timeMultiplier)
                {
                ...
                if (millis() - _timer >= Settings::getDelay() * timeMultiplier)
                {
                    _stepNumber++;
                    Write("STEP ", _stepNumber);
        ...
        void runNonstop()
        {
            ...
            if (_currentState == Beginning && millis() - _timer >= Settings::getExposure() * timeMultiplier)
            {
                _stepNumber++;
                Write("STEP ", _stepNumber);
                digitalWrite(SHUTTER, CAMERA_HIGH); // make first photo
            ...
            if (_currentState == Exposure && millis() - _timer >= releaseShutterDelay * timeMultiplier)
            {
                digitalWrite(SHUTTER, CAMERA_LOW); // release shutter
                _currentState = Move;
            }
        ...
        void runVideo()
        {
            ...
            {
                if (mover.getState() != Mover::Correction && millis() - _timer >= 50 * timeMultiplier)
                {
