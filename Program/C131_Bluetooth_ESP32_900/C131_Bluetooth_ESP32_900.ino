...
BLECharacteristic* pUpdatesCharacteristic = NULL;
...
#endif
    pUpdatesCharacteristic->setValue((uint8_t*)strBuf, strlen(strBuf));
    pUpdatesCharacteristic->notify();
}
...
class Settings
{
    ...
    private:
        ...
        static float getTimeOfTurn(unsigned char pwm)
        {
            ...
            {
            		255, 255, 255, 236, 221, 207, 196, 185, 176, 169, 161, 154, 148, 142, 136, 131, 126, 123, 119, 115,
            		111, 108, 105, 102, 99, 97, 94, 92, 89, 87, 85, 83, 81, 80, 78, 76, 74, 73, 71, 70,
            		69, 68, 67, 65, 64, 63, 62, 61, 60, 59, 58, 57, 56, 56, 55, 54, 53, 53, 52, 51,
            		51, 50, 49, 49, 48, 47, 47, 46, 46, 45, 44, 44, 43, 43, 43, 42, 42, 41, 41, 40,
            		40, 39, 39, 39, 38, 38, 38, 37, 37, 37, 36, 36, 36, 35, 35, 35, 34, 34, 34, 33,
            		33, 33, 33, 32, 32, 32, 32, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29, 29, 29, 29,
            		29, 28, 28, 28, 28, 28, 27, 27, 27, 27, 27, 27, 26, 26, 26, 26, 26, 26, 26, 25,
            		25, 25, 25, 25, 25, 25, 24, 24, 24, 24, 24, 24, 24, 24, 23, 23, 23, 23, 23, 23,
            		23, 23, 22, 22, 22, 22, 22, 22, 22, 22, 22, 21, 21, 21, 21, 21, 21, 21, 21, 21,
            		21, 21, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 19, 19, 19, 19, 19, 19,
...
void setup()
{
    ...
    ledcSetup(MOTOR_CHANNEL1, 1000, 8);
    ledcAttachPin(MOTOR2, MOTOR_CHANNEL2);
    // frequency 1000 Hz, resolution 8 bit
    ledcSetup(MOTOR_CHANNEL2, 1000, 8);
    ...
    BLEDevice::init("Rotating Table");
    BLEServer* pServer = BLEDevice::createServer();
    pServer->setCallbacks(&serverCallbacks);
    BLEService* pService = pServer->createService(uuidOfService);
    pUpdatesCharacteristic = pService->createCharacteristic(
        updatesCharacteristicUuid, BLECharacteristic::PROPERTY_NOTIFY);
    pWriteCharacteristic = pService->createCharacteristic(
        writeCharacteristicUuid, BLECharacteristic::PROPERTY_WRITE);
    pWriteCharacteristic->setCallbacks(&writeCallbacks);
