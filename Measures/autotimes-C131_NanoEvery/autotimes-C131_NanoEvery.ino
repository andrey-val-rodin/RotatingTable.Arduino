...
void Runner::runAutomatic()
{
    ...
    if (currentState == Beginning && millis() - timer >= Settings::getExposure() * timeMultiplier)
    {
        stepNumber++;
    ...
    if (currentState == Exposure && millis() - timer >= Settings::getExposure() * timeMultiplier)
    {
    ...
    if (currentState == Delay && millis() - timer >= Settings::getDelay() * timeMultiplier)
    {
        stepNumber++;
        display(mode, stepName);
...
class TurnMeasurer
{
    public:
        ...
        String getOutput()
        {
            float time = (_stop - _start) / 1000.0 / timeMultiplier;
            return "\t" + String(time);
        }
